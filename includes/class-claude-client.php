...
class FCE_Claude_Client {
	...
	private static function post( $api_key, array $body ) {
		...
		// Walk the content array, concatenating every text block. Citation
		// metadata on each block is intentionally ignored here — Claude's
		// structured citations attach at the block level and don't reach
		// inside JSON string values, so they don't help with the narrative
		// content we're after. We instead instruct Claude in the system
		// prompt to use Markdown link syntax inside narrative strings, and
		// the data mapper strips any stray <cite> tags defensively. See
		// the v0.5.0 commits for the history of this decision.
		$text         = '';
		$search_count = 0;
